...
<body style="margin:0;padding:0;background:#FAF7F0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#1a1a1a;">
<div style="display:none;max-height:0;overflow:hidden;opacity:0;font-size:1px;line-height:1px;color:#FAF7F0;">${esc(preheader)}</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FAF7F0;">
  <tr><td align="center" style="padding:32px 16px;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 4px 24px rgba(61,46,31,0.08);">
      <tr><td style="background:#4F5E3A;padding:24px 28px;">
        <div style="font-family:Georgia,'Times New Roman',serif;color:#FAF7F0;font-size:20px;letter-spacing:0.5px;">Le Clos des Cocales</div>
        <div style="color:#C97B4A;font-size:11px;letter-spacing:2px;text-transform:uppercase;margin-top:4px;">Espondeilhan · Hérault</div>
      </td></tr>
      <tr><td style="padding:32px 28px;line-height:1.6;font-size:16px;color:#1a1a1a;">
        ${body}
      </td></tr>
      <tr><td style="background:#FAF7F0;padding:20px 28px;text-align:center;font-size:12px;color:#6b6f5d;line-height:1.5;">
        <a href="${SITE_URL}" style="color:#4F5E3A;text-decoration:none;font-weight:600;">clos-des-cocales.fr</a>
        &nbsp;·&nbsp; <a href="tel:${PHONE_TEL}" style="color:#4F5E3A;text-decoration:none;">${PHONE_DISPLAY}</a>
...
function btn(href: string, label: string, primary = true) {
  const bg = primary ? "#4F5E3A" : "#ffffff";
  const color = primary ? "#FAF7F0" : "#4F5E3A";
  const border = primary ? "#4F5E3A" : "#4F5E3A";
...
function confirmationHtml(lead: LeadPayload) {
  ...
  const body = `
    <p style="margin:0 0 8px;font-size:13px;letter-spacing:2px;text-transform:uppercase;color:#C97B4A;font-weight:600;">Demande reçue</p>
    <h1 style="font-family:Georgia,'Times New Roman',serif;font-size:28px;line-height:1.2;margin:0 0 18px;color:#4F5E3A;font-weight:500;">
      Merci ${first}.
    </h1>
    ...
    <div style="background:#FAF7F0;border-radius:14px;padding:18px 20px;margin:24px 0;">
      <div style="font-size:13px;text-transform:uppercase;letter-spacing:1.5px;color:#C97B4A;font-weight:600;margin-bottom:10px;">Les prochaines étapes</div>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        <tr><td style="padding:6px 0;font-size:15px;"><strong style="color:#4F5E3A;">1.</strong> &nbsp;Échange téléphonique pour comprendre votre projet</td></tr>
        <tr><td style="padding:6px 0;font-size:15px;"><strong style="color:#4F5E3A;">2.</strong> &nbsp;Envoi de la brochure, plan de masse et tarifs</td></tr>
        <tr><td style="padding:6px 0;font-size:15px;"><strong style="color:#4F5E3A;">3.</strong> &nbsp;Visite sur place à Espondeilhan</td></tr>
      </table>
    </div>
    ...
    <ul style="padding-left:18px;margin:0 0 20px;">
      <li style="margin-bottom:6px;"><a href="${SITE_URL}/outils/simulateur-pret" style="color:#4F5E3A;">Simulateur de prêt immobilier</a></li>
      <li style="margin-bottom:6px;"><a href="${SITE_URL}/outils/budget-total" style="color:#4F5E3A;">Budget total terrain + maison</a></li>
      <li style="margin-bottom:6px;"><a href="${SITE_URL}/guide/acheter-terrain-a-batir" style="color:#4F5E3A;">Guide : acheter un terrain à bâtir</a></li>
    </ul>

    <p style="margin:24px 0 0;color:#6b6f5d;font-size:14px;">À très bientôt,<br/><strong style="color:#4F5E3A;">L'équipe du Clos des Cocales</strong></p>
  `;
  return shell(
...
function adminNotifHtml(lead: LeadPayload) {
  ...
  const rows = [
    ["Nom", lead.name],
    ["Email", `<a href="mailto:${esc(lead.email)}" style="color:#4F5E3A;">${esc(lead.email)}</a>`],
    lead.phone
      ? ["Téléphone", `<a href="tel:${esc(lead.phone)}" style="color:#4F5E3A;">${esc(lead.phone)}</a>`]
      : null,
  ...
  const body = `
    <p style="margin:0 0 8px;font-size:13px;letter-spacing:2px;text-transform:uppercase;color:#C97B4A;font-weight:600;">Nouveau lead</p>
    <h1 style="font-family:Georgia,'Times New Roman',serif;font-size:24px;margin:0 0 18px;color:#4F5E3A;font-weight:500;">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee5d6;border-radius:12px;overflow:hidden;margin:16px 0 22px;">
      ${rows
        ...
        <tr style="background:${i % 2 ? "#fbf8f1" : "#ffffff"};">
          <td style="padding:10px 14px;font-size:13px;color:#6b6f5d;width:140px;vertical-align:top;">${k}</td>
    ...
    <p style="margin:24px 0 0;color:#6b6f5d;font-size:13px;">Lead reçu via ${esc(lead.source ?? "site")} · ${new Date().toLocaleString("fr-FR", { timeZone: "Europe/Paris" })}</p>
  `;
...
function followUpHtml(lead: LeadPayload) {
  ...
  const body = `
    <p style="margin:0 0 8px;font-size:13px;letter-spacing:2px;text-transform:uppercase;color:#C97B4A;font-weight:600;">Suite à votre demande</p>
    <h1 style="font-family:Georgia,'Times New Roman',serif;font-size:26px;line-height:1.25;margin:0 0 16px;color:#4F5E3A;font-weight:500;">
    ...
    <div style="background:#FAF7F0;border-radius:14px;padding:18px 20px;margin:0 0 20px;">
      <div style="font-size:13px;text-transform:uppercase;letter-spacing:1.5px;color:#C97B4A;font-weight:600;margin-bottom:10px;">Ressources utiles</div>
      <ul style="padding-left:18px;margin:0;">
        <li style="margin-bottom:6px;"><a href="${SITE_URL}/outils/budget-total" style="color:#4F5E3A;">Calculer votre budget total terrain + maison</a></li>
        <li style="margin-bottom:6px;"><a href="${SITE_URL}/outils/simulateur-pret" style="color:#4F5E3A;">Simulateur de prêt</a></li>
        <li style="margin-bottom:6px;"><a href="${SITE_URL}/guide/acheter-terrain-a-batir" style="color:#4F5E3A;">Guide pratique de l'achat de terrain</a></li>
      </ul>
    </div>
    ...
    <p style="margin:24px 0 0;color:#6b6f5d;font-size:14px;">À très vite,<br/><strong style="color:#4F5E3A;">L'équipe du Clos des Cocales</strong></p>
  `;
  return shell(
